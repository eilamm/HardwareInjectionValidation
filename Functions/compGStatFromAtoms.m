...
% This function will compute the daily and cumulative G-stat values for the input 'date'
% and for all pulsars. It is not meant for direct use, but rather for use through the 
% wrapper functions compGStatFromAtoms_wrapperToday, compGStatFromAtoms_wrapperTotal 

function compGStatFromAtoms(date)
	pulsars = [0:12, 14]; 				% Skip pulsar 13
	projPath = '/home/eilam.morag/hw_injection/Hardware_Injection_2016'; 
	for p = pulsars
		fprintf('Calculating G-stat for Pulsar %d, %s...\n', p, date.date2str());
		...
		%% Parse the atoms and compute the G-stat values for pulsar 'p'
		fprintf('\tDaily G-stat -- ');
		dailyGStatsFromAtoms = compGStat(dailyAtomsPath);	% Need to modify parseAtoms so it's a func that returns 2F values
		fprintf('\tCumulative G-stat -- ');
		cumulGStatsFromAtoms = compGStat(cumulAtomsPath);

		% Get the loudest G-stat
		dailyLoudestGStat = max(dailyGStatsFromAtoms);
		cumulLoudestGStat = max(cumulGStatsFromAtoms);
	
		%% Output cumulative and daily G-stat to file
		outputPath = sprintf('%s/output/Pulsar%d/%s', projPath, p, date.date2str_nospace());
		filename = sprintf('%s/GStatFromAtoms_Pulsar%d_%s.txt', outputPath, p, date.date2str_nospace());
		fprintf('Outputting loudest G-stat candidates to %s... ', filename);
		fileID = fopen(filename, 'w');
		fprintf(fileID, 'Loudest G-stat for Pulsar %d on %s\n', p, date.date2str());
		fprintf(fileID, 'daily\tcumulative\n%f\t%f', dailyLoudestGStat, cumulLoudestGStat);
		fclose(fileID);
		fprintf('done\n\n');
...
% and returns the G-stat values for all the frequency bins.
function G = compGStat(pathToAtoms)
...
%	filename = '/home/eilam.morag/hw_injection/Hardware_Injection_2016/scripts/Prefix_tRef751680013_RA0.652645832_DEC-0.514042406_Freq849.083308009024_f1dot-3e-10.dat';
	...
	%		fprintf('File exists\n');
		...
		G = D_inv*( B*abs(Fa_t)^2 + A*abs(Fb_t)^2 - 2*C*real(Fa_t*conj(Fb_t)) );
		G(k) = 2*F;
